// pages/api/yahoo-test.js
// Probe a single Yahoo symbol quickly to see if it returns points.
...
export default async function handler(req, res) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      return res
        .status(200)
        .json({ ok: true, symbol, status, attempt, points: rows.length, sample: rows.slice(-3) });
    } catch (e) {
  ...
  return res
    .status(200)
    .json({ ok: false, symbol, status, error: err, points: rows.length });
}
