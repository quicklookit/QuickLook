...
function mergeSeries(seriesList) {
  ...
  for (const s of seriesList) for (const row of s.data) {
...
async function fetchGoogleTrendOverSegments(keyword, startTime, endTime, segmentDays=30) {
  const segs=[]; for (let t=new Date(startTime); t<endTime; ) {
    const s=new Date(t), e=new Date(Math.min(s.getTime()+segmentDays*86400000,endTime.getTime()));
    segs.push({s,e}); t=new Date(e.getTime()+86400000);
  }
  const parts = await Promise.all(segs.map(async ({s,e})=>{
    const json = await googleTrends.interestOverTime({ keyword:keyword, startTime:s, endTime:e, geo:'', hl:'en-US', timezone:0, granularTimeResolution:true });
    const parsed = JSON.parse(json); return parsed?.default?.timelineData ?? [];
  }));
...
async function fetchYahooDaily(symbol, days) {
  ...
  const r=await fetch(url); if(!r.ok) throw new Error(`Yahoo ${symbol} ${r.status}`);
...
async function fetchCoinGeckoBTC(days) {
  ...
  const j=await r.json(); return (j.prices||[]).map(([t,price])=>({ date:new Date(t).toISOString().slice(0,10), value:price }));
}

export default async function handler(req,res){
  res.setHeader('X-Route-Impl','pages-v1');
  res.setHeader('Cache-Control','no-store');

  // FORCE long window while validating
  const days=180, endTime=new Date(), startTime=new Date(Date.now()-days*86400000);

  const searchKeywords=['cosmetics','lipstick','male underwear','PMI index','interest rates','mortgage lending','credit card debt','job openings','house prices','European Central Bank','Federal Reserve','Bank for International Settlements'];
  const limit=pLimit(3);

  try{
    const googleSeries=await Promise.all(searchKeywords.map(kw=>limit(async()=>{
      try{
    ...
    const marketSeries=[
      {name:'gold',   data:g.map((r,i)=>({date:r.date, raw:r.value, value:normalizeArray(g.map(x=>x.value))[i]}))},
      {name:'nasdaq', data:n.map((r,i)=>({date:r.date, raw:r.value, value:normalizeArray(n.map(x=>x.value))[i]}))},
      {name:'bitcoin',data:b.map((r,i)=>({date:r.date, raw:r.value, value:normalizeArray(b.map(x=>x.value))[i]}))}
    ];
    ...
    if(req.query.debug==='1'){
      ...
      return res.status(200).json({ signature:'fetch-trends-v1', rows:merged.length, keys, countsPerSeries:counts, sample:merged.slice(0,2) });
    }
