// pages/api/fetch-trends.js (diagnostic build)
...
const SERIES = [
  ...
  // two sanity checks to see if *any* extra ticker works:
...
export default async function handler(req, res) {
  ...
  const diagnostics = [];   // will hold per-series status
  ...
  try {
    const series = await Promise.all(
      SERIES.map(s =>
        limit(async () => {
          ...
          let rows = [];
          if (s.kind === 'yahoo') {
            rows = await fetchYahooDaily(s.symbol, days, diag);
    ...
    if (req.query.debug === '1') {
      ...
      return res.status(200).json({
        signature: 'fetch-trends-proxy-diag',
        rows: merged.length,
        series: keys,
        countsPerSeries: counts,
        diagnostics,             // <--- why each series passed/failed
